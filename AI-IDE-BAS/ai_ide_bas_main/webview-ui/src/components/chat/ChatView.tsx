...
const ChatViewComponent: React.ForwardRefRenderFunction<ChatViewRef, ChatViewProps> = (
	...
	const handleFastApiPost = useCallback(() => {
		setFastApiResult("Sending request...")
		vscode.postMessage({ type: "fastApiPost" })
	}, [])
		...
		(e: MessageEvent) => {
			...
			switch (message.type) {
				...
				case "condenseTaskContextResponse":
					...
					break
				case "fastApiPostResult":
					if (message.text) {
						const isError = message.text.startsWith("Error")
						setFastApiResult(isError ? message.text : `Success: ${message.text}`)
					}
					break
			}
		...
		<div
			...
			{task ? (
				...
				<div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-4 relative">
					{/* FastAPI Test */}
					<div className="mx-3 mt-3 mb-2 p-4 border border-vscode-sideBarSectionHeader-border rounded-lg bg-vscode-sideBar-background">
						<div className="flex items-center gap-2 mb-3 text-vscode-descriptionForeground text-xs font-bold uppercase tracking-wider">
							<i className="codicon codicon-api"></i>
							<span>FastAPI Integration</span>
						</div>
						<VSCodeButton onClick={handleFastApiPost} appearance="primary" className="w-full h-10 font-bold">
							FAST API POST
						</VSCodeButton>
						{fastApiResult && (
							<div
								className="mt-3 p-3 text-xs rounded border bg-vscode-editor-background break-all"
								style={{
									color: fastApiResult.startsWith("Error") ? "var(--vscode-errorForeground)" : "var(--vscode-charts-green)",
